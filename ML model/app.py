...
def predict(img):
    ...
    if np.argmax(pred, axis=1)[0] == 0:
        out_pred = "Brain Tumor: Symptoms include unexplained weight loss, double vision or a loss of vision, increased pressure felt in the back of the head, dizziness and a loss of balance, sudden inability to speak, hearing loss, weakness or numbness that gradually worsens on one side of the body."
    else:
        out_pred = "Normal"
